...
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="myform2">
                <div class="modal-body">
                    用户id<input id="edit_uid" type="text" name="uid"/>
                    <br/>
                    用户名<input id="edit_username" type="text" name="username"/>
                    <br/>
                    商品id<input id="edit_pid" type="text" name="pid"/>
            ...
            columns: [
                ...
                {title:'操作',formatter:function (value,row,index) {
                    ...
                    return "<button class='btn btn-info' onclick='edit_user("+product_json+")'>修改</button>" +
                        "<button class='btn btn-danger delete_user' onclick='delete_user("+row.id+")'>删除</button>"
                }} ],
            //要绑定的数据
            //这里绑定的是死数据，如果使用的客户端分页的json格式：[{},{},{}]
            //如果使用的服务端分页,json格式如下：{total:98,rows:[{},{}]}
            //请求地址
            url: '${pageContext.request.contextPath}/product/cart3',
            //请求方式
            method: 'post',
//              data: [{ id: 1, name: 'Item 1', price: '$1' }, { id: 2, name: 'Item 2', price: '$2' }, { id: 3, name: 'Item 3', price: '$3' }],
    ...
    $(function(){
        $("#addbtn").click(function(){
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/product/addcart",
                data:param,
                dataType:"text",
                type:"post",
    ...
    $(function(){
        $("#editbtn").click(function(){
            ...
            console.log(param);
            $.post(
                "${pageContext.request.contextPath}/product/editcart",
                param,
                function(rec){
                    if(rec==0){
                        alert("数据不合法")
...
<script type="text/javascript">
    ...
    function delete_user(id){
        console.log(id);
        $.post(
            "${pageContext.request.contextPath}/product/deletecart/"+id,
            //{'_method': 'DELETE'},
            function () {
                $("#mytable").bootstrapTable(('refresh'));
            }
    ...
    function edit_user(product_json){
        $("#edit_uid").val(product_json.uid);
        $("#edit_username").val(product_json.username);
        $("#edit_id").val(product_json.id);
